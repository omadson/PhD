function makeLineChart(dataset, xName, yObjs, axisLables, height, width, chart_type) {
    var chartObj = {};
...
// Build line building functions
    ...
    for (var yObj in yObjs) {
        yObjs[yObj].line = d3.svg.line().interpolate("cardinal").x(function (d) {
			//console.log(months[parseInt(chartObj.xFunct(d))]);
...
// Render the chart
    chartObj.render = function () {
        ...
        //console.log(chartObj.xAxisLable);

        chartObj.svg.append("g").attr("class", "y axis").call(chartObj.yAxis).append("text").attr("class", "label").attr("transform", "rotate(-90)").attr("y", -52).attr("x", -chartObj.height / 2).attr("dy", ".1em").style("text-anchor", "middle").text(chartObj.yAxisLable);
